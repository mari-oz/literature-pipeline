...
<head>
  ...
  <style>
    ...
    input, select { padding:.75rem; font-size:1rem; }
...
<body>
  ...
  <p class="muted">Match = raw FTS5 syntax, Phrase = exact phrase, Prefix = token*, Near = proximity search.</p>
  ...
  <script>
    ...
    function normalizeRanks(items) {
      ...
      const vals = items.map(x => Number(x.score));
      const min = Math.min(...vals);
      const max = Math.max(...vals);
      return items.map(item => {
        const score = Number(item.score);
        const pct = max === min ? 100 : Math.max(5, Math.round((max - score) / (max - min) * 100));
    ...
    function render(items) {
      ...
      for (const item of normalizeRanks(items)) {
        ...
        el.innerHTML = `
          <h2>${item.title}</h2>
          <div class="meta">DOI: ${item.doi || '-'} | Category: ${item.category || '-'} | Date: ${item.published_date || '-'} | Score: ${Number(item.score).toFixed(4)}</div>
          <p>${item.snippet || ''}</p>
